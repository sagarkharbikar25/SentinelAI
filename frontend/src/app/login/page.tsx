  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data.success && res.data.data.accessToken) {
        localStorage.setItem('sentinel_token', res.data.data.accessToken);
        localStorage.setItem('sentinel_user', JSON.stringify(res.data.data.user));
        router.push('/');
      } else {
    ...
    <div className="min-h-[80vh] flex items-center justify-center p-4">
      <div className="w-full max-w-md space-y-6">
        <div className="text-center space-y-2">
          ...
          <h2 className="text-2xl font-extrabold text-white tracking-wide">Sign in to SentinelAI</h2>
          <p className="text-xs text-slate-400 font-medium">Enterprise AI Agent Security & Governance Platform</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="p-6 rounded-2xl bg-[#13151C] border border-[#232733] space-y-4 shadow-xl">
          <div>
